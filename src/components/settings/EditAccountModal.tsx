...
export function EditAccountModal({ isOpen, onClose, account }: EditAccountModalProps) {
  ...
  const isGoogle = providerLower === 'gmail' || providerLower === 'google' || oauthProviderLower === 'google';
  const isMicrosoft = providerLower === 'outlook' || providerLower === 'microsoft' || providerLower === 'office365' || oauthProviderLower === 'microsoft';
  const isOauth = oauthProviderLower !== '' || isGoogle || isMicrosoft;
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                  ...
                  <p className="text-sm text-gray-500 mb-6">
                    This account is connected securely via {isMicrosoft ? 'Microsoft Outlook' : isGoogle ? 'Google Workspace / Gmail' : 'OAuth'}. To fix connection issues, you must reauthorize access directly through them.
                  </p>
                  <button
                    type="button"
                    onClick={handleOauthReauthorize}
                    className="inline-flex items-center justify-center rounded-md border border-transparent bg-accent-600 px-6 py-3 text-sm font-medium text-white shadow-sm hover:bg-accent-700 font-semibold"
                  >
                    {isGoogle ? 'Reauthorize with Gmail' : isMicrosoft ? 'Reauthorize with Microsoft' : 'Reauthorize Account'}
                  </button>
                </div>
              </div>
            ) : (
